import { repairSourcePaths } from '../../core/repair-source-paths.js';
import { wrapStructuredResult, wrapThrownError } from '../shared/error.js';

export const name = 'prunemem_repair_source_paths';

export const description =
  'Repair missing source-path references in the memory registry. ' +
  'If write is true, writes repaired source paths to disk. ' +
...
export const inputSchema = {
  ...
  properties: {
    ...
    write: {
      type: 'boolean',
      description:
        'If true, writes repaired source paths to disk. Defaults to false (dry-run). ' +
...
export async function handler(args) {
  ...
  try {
    const result = await repairSourcePaths(params);
