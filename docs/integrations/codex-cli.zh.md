...
这是基础 v0.3.0 MCP 的预期行为。Codex CLI 何时、如何调用 PruneMem tool 的主动使用指引，现已由 PruneMem 治理 skill（`skills/prunemem-memory-governance/SKILL.md`）和 agent playbook（`docs/agent-playbook.md`）提供。
