...
| `getWorkingState` | `prunemem_get_working_state` | read | 否 | — | `workspace`, `input`, `preset`, `override`, `paths` | 否 | 纯读。返回 parsed working state JSON。Phase C 第 1 批。 |
| `maintain` | `prunemem_maintain` | write | 否 | — | `workspace`, `write`, `strict`, `repair_source_paths`, `timeout_ms`, `preset`, `override`, `paths` | 是 | 组合工具：validate → curator-apply → [repair] → validate。`timeout_ms` 已废弃但保留兼容。Phase C 第 3 批。 |
| `repairSourcePaths` | `prunemem_repair_source_paths` | write | 否 | — | `workspace`, `write`, `preset`, `override`, `paths` | 是 | 从 registry 反推缺失的 pipeline artifact 并写 placeholder。`write` 默认 `false`。Phase C 第 2 批。 |
| `runExtract` | `prunemem_run_extract` | write | 否 | — | `workspace`, `input`, `output`, `mock`, `preset`, `override`, `paths` | **TBD** | 无条件写盘（无 `write` 开关）。需先解决 `mcp-design.md` R1 再暴露。Phase C 第 4 批。 |
| `runJudge` | `prunemem_run_judge` | write | 否 | — | `workspace`, `input`, `output`, `mock`, `preset`, `override`, `paths` | **TBD** | 无条件写盘（无 `write` 开关）。需先解决 `mcp-design.md` R1 再暴露。Phase C 第 4 批。 |
| `runSamplePipeline` | `prunemem_run_sample_pipeline` | write | 否 | — | `workspace`, `mock`, `write`, `preset`, `override`, `paths` | 是 | 组合工具：extract → judge → update-registries。`write` 默认 `false`，透传给 `updateRegistries`。`mock` 参数支持 deterministic test。Phase C 第 3 批。 |
| `updateRegistries` | `prunemem_update_registries` | write | 否 | — | `workspace`, `judged`, `source_paths`, `memory_id`, `channel`, `agent`, `write`, `preset`, `override`, `paths` | 是 | `write` 默认 `false`（0.4.0 breaking change）。向 registry jsonl 插入 judged items。Phase C 第 2 批。 |
| `updateWorkingState` | `prunemem_update_working_state` | write | 否 | — | `workspace`, `input`, `state`, `write`, `preset`, `override`, `paths` | 是 | `write` 默认 `false`。合并 delta 到 working state 并可选写盘。Phase C 第 2 批。 |
| `validateMaintenance` | `prunemem_validate_maintenance` | read | 否 | — | `workspace`, `strict`, `preset`, `override`, `paths` | 否 | 纯读。检查 registry 一致性、source_paths 可达性、MEMORY.md 重复 bullet。Phase C 第 1 批。 |
