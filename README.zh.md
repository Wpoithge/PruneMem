...
PruneMem 不替代你的向量存储或检索机制——它在其上游。你把会话记录或事实喂给 PruneMem，它判断该留什么、放哪，并维护干净、去重、带生命周期管理的登记簿。**查询时，检索由你自己接入**：把向量库或搜索索引（例如 QMD）指向 PruneMem 维护的那些文件，按宿主原有方式查询即可。一句话——PruneMem 负责*治理*（留什么、放哪、留多久），检索层负责*召回*。
